...
# Run `python -m py_compile Neural_Identity_Matrix_original_Test_V24.35.py` to check syntax before execution
# Ensure dataset.csv, previous_names.csv, upper_clothing.csv, lower_clothing.csv, footwear.csv, style_themes.csv, locations.csv, overall_themes.csv, artist_names.csv, genres.csv, themes.csv are in the project directory
...
try:
    ...
    # Validate non-empty CSV files
    if len(artist_names_df) == 0 or len(genres_df) == 0 or len(themes_df) == 0:
        raise ValueError("One or more playlist CSV files (artist_names.csv, genres.csv, themes.csv) is empty. Please ensure they contain valid data.")
except FileNotFoundError as e:
    print(f"Error: {e}. Please ensure artist_names.csv, genres.csv, and themes.csv are in the project directory.")
    raise
...
def generate_flux_image(selected_identity, df_identities, allow_nsfw=False, style_theme="Cyberpunk", location="Cosmic Nebula", overall_theme="Ethereal Dreamscape"):
    ...
    try:
        ...
        prompt = (
            ...
            f"preference, with {row['Hair color'].lower()} hair and {row['Eye color'].lower()} eyes, "
...
# NEW: Generate audio prompt for ACE-Step
def generate_audio_prompt(selected_identity, df_identities, style_theme, location, overall_theme):
    ...
    try:
        ...
        genre = playlist.split()[1].lower()  # e.g., "ambient" from "Luna Vox’s Ambient Ethereal Drift Mix"
        prompt = (
...
def suggest_caption(row):
    ...
    return f"{random.choice(traits)} shines in V24.35! 🌌 #CosmicDreams #AIArt"
        ...
        for i in range(num_identities):
            ...
            if random.random() < 0.02:
                ...
                if cosmic_pet == 'Melody Finch named Harmony' and random.random() < 0.5:
                    trait = random.choice(['inspiring', 'uplifting'])
                    cosmic_pet = f"{cosmic_pet} ({trait})"
...
.dataframe tr:has(td:contains("Melody Finch named Harmony")) {
    box-shadow: 0 0 10px rgba(255, 255, 50, 0.7) !important;
    animation: pulse 1.5s infinite;
}
.dataframe tr:has(td:contains("Melody Finch named Harmony")):hover::after {
    content: "A shimmering finch inspiring musical creativity";
    position: absolute;
    background: #0a0a28;
    color: #00ffcc;
    padding: 5px;
...
# UPDATED: Gradio interface with playlist filter and audio prompt components
with gr.Blocks(css=custom_css, theme="default") as demo:
    ...
    with gr.Row():
        ...
        playlist_filter = gr.Dropdown(
            choices=['All'] + themes_list,
            value='All',
            label="Filter by Playlist Theme"
        )
    ...
    with gr.Tabs():
        with gr.Tab(label="Identity Generator"):
            ...
            with gr.Row():
                ...
                generate_audio_button = gr.Button("Generate Audio Prompt for ACE-Step")
            ...
            audio_prompt_output = gr.Textbox(
                label="Audio Prompt for ACE-Step",
                placeholder="Click 'Generate Audio Prompt' to create a prompt for ACE-Step.",
                interactive=False
            )
    ...
    def filter_identities(df_identities, profession_filter, playlist_filter):
        if df_identities is None or df_identities.empty:
            return df_identities
        filtered_df = df_identities
        if profession_filter != 'All':
            filtered_df = filtered_df[filtered_df['Profession'] == profession_filter]
        if playlist_filter != 'All':
            filtered_df = filtered_df[filtered_df['Cosmic Playlist'].str.contains(playlist_filter, case=False, na=False)]
        return filtered_df
    ...
    generate_audio_button.click(
        ...
        inputs=[identity_dropdown, output, style_theme_dropdown, location_dropdown, overall_theme_dropdown],
        outputs=audio_prompt_output
    )
    ...
    clear_button.click(
        ...
        outputs=[output, download_button, download_plot_output, identity_dropdown, image_output, progress_bar, status_message, loss_plot, log_output, image_output, image_status, gallery_output, caption_input, share_status, nsfw_warning, audio_prompt_output]
    )
    ...
    # Add filter update event
    profession_filter.change(
        fn=filter_identities,
        inputs=[output, profession_filter, playlist_filter],
        outputs=output
    )
    playlist_filter.change(
        fn=filter_identities,
        inputs=[output, profession_filter, playlist_filter],
        outputs=output
    )
